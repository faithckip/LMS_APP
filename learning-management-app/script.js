...
function displayCourseContent(courseContent){
    ...
    //Loop through the module and display them
    courseContent.modules.forEach(module => {
        ...
        <!--Add more elements as needed (e.g. VideoColorSpace, quizzes)-->
        `;
...
function fetchFullName() {
    //Make AJAX request to fetch the users's full name from the server
    fetch('/get-fullname')
    .then(response => {
        if(!response.ok) {
            throw new Error('Network response was not ok');
